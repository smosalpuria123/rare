...
router.get('/result/:imdbId', function(req, res, next) {
    ...
    const url = 'https://api.themoviedb.org/3/movie/'+imdbId+'?api_key='+apiKey+'&language=en-US&append_to_response=images,videos,credits,recommendations,popular&include_image_language=en,null';
    axios.get(url)
        .then(function (response) {
            // handle success
            res.send(response.data);
